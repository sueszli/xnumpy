...
def rmsnorm_fwd(x: list[list[float]]) -> tuple[list[list[float]], list[float]]:
    ...
    for i in range(n):
        row = x[i]
        scale = (sum(v * v for v in row) * inv_d + 1e-5) ** -0.5
        rms[i] = scale
        out_i = out[i]
        for j in range(d):
            out_i[j] = row[j] * scale
...
def rmsnorm_bwd(dout: list[list[float]], x: list[list[float]], rms: list[float]) -> list[list[float]]:
    ...
    for i in range(n):
        ...
        dot = sum(do[j] * row[j] for j in range(d))
        dx_i = dx[i]
        for j in range(d):
...
def layer_fwd(x: list[list[float]], params: dict, li: int) -> tuple[list[list[float]], dict]:
    _sum = sum
    _exp = math.exp
    _rD = range(N_EMBED)
    ...
    for i in range(n):
        ...
        for j in _rD:
            ...
            q[h_idx][i][d_idx] = _sum(xn_i[d] * wq_j[d] for d in _rD)
            k[h_idx][i][d_idx] = _sum(xn_i[d] * wk_j[d] for d in _rD)
            v[h_idx][i][d_idx] = _sum(xn_i[d] * wv_j[d] for d in _rD)
    ...
    for h in range(N_HEAD):
        ...
        for i in range(n):
            q_h_i = q_h[i]
            qk_i = [_sum(q_h_i[d] * k_h[j][d] for d in _rH) * inv_scale if j <= i else -1e10 for j in range(n)]
            max_val = max(qk_i)
            ...
            for d in _rH:
                flat_i[h_off + d] = _sum(aw_i[j] * v_h[j][d] for j in range(n))

    x = [[0.0] * N_EMBED for _ in range(n)]
    for i in range(n):
        flat_i, pre_i, x_i = attn_out_flat[i], x_pre_attn[i], x[i]
        for j in _rD:
            x_i[j] = _sum(flat_i[d] * wo[j][d] for d in _rD) + pre_i[j]
    ...
    for i in range(n):
        ...
        for j in _rM:
            val = _sum(xn_i[d] * fc1[j][d] for d in _rD)
            hp_i[j] = val
    ...
    for i in range(n):
        ...
        for j in _rD:
            x_i[j] = _sum(hv_i[d] * fc2[j][d] for d in _rM) + pre_i[j]
...
def layer_bwd(dx: list[list[float]], cache: dict, params: dict, li: int) -> tuple[list[list[float]], dict]:
    _sum = sum
    _rD = range(N_EMBED)
    _rH = range(N_EMBED // N_HEAD)
    ...
    for j in _rD:
        ...
        h_cache = cache["h"]
        for d in _rM:
            dfc2_j[d] = _sum(dx[i][j] * h_cache[i][d] for i in range(n))
    ...
    for i in range(n):
        ...
        for j in _rM:
            val = _sum(dx_i[d] * fc2[d][j] for d in _rD)
    ...
    for j in _rM:
        dfc1_j = dfc1[j]
        for d in _rD:
            dfc1_j[d] = _sum(dh_pre[i][j] * xn_mlp_cache[i][d] for i in range(n))

    dxn_mlp = [[0.0] * N_EMBED for _ in range(n)]
    for i in range(n):
        dhp_i, dxn_i = dh_pre[i], dxn_mlp[i]
        for d in _rD:
            dxn_i[d] = _sum(dhp_i[j] * fc1[j][d] for j in _rM)
    ...
    for j in _rD:
        dwo_j = dwo[j]
        for d in _rD:
            dwo_j[d] = _sum(dx[i][j] * attn_out_flat_cache[i][d] for i in range(n))

    dattn_out_flat = [[0.0] * N_EMBED for _ in range(n)]
    for i in range(n):
        dx_i, dat_i = dx[i], dattn_out_flat[i]
        for d in _rD:
            dat_i[d] = _sum(dx_i[j] * wo[j][d] for j in _rD)
    ...
    for h in range(N_HEAD):
        ...
        for i in range(n):
            dv_h_i = dv_h[i]
            for d in _rH:
                dv_h_i[d] = _sum(aw_h[j][i] * dattn_out_flat[j][h_off + d] for j in range(n))
        for i in range(n):
            dat_i, daw_h_i = dattn_out_flat[i], daw_h[i]
            for j in range(n):
                daw_h_i[j] = _sum(dat_i[h_off + d] * v_h[j][d] for d in _rH)
    ...
    for h in range(N_HEAD):
        ...
        for i in range(n):
            ...
            s = _sum(daw_i[j] * aw_i[j] for j in range(n))
            dla_i = dla_h[i]
            for j in range(n):
    ...
    for h in range(N_HEAD):
        ...
        for i in range(n):
            ...
            for d in _rH:
                dq_i[d] = _sum(dla_i[j] * k_h[j][d] for j in range(n))
                dk_i[d] = _sum(dla_h[j][i] * q_h[j][d] for j in range(n))
    ...
    for j in _rD:
        ...
        for k_dim in _rD:
            dwq_j[k_dim] = _sum(dq_h[i][d_idx] * xn_attn_cache[i][k_dim] for i in range(n))
            dwk_j[k_dim] = _sum(dk_h[i][d_idx] * xn_attn_cache[i][k_dim] for i in range(n))
            dwv_j[k_dim] = _sum(dv_h[i][d_idx] * xn_attn_cache[i][k_dim] for i in range(n))
...
def forward_backward(params: dict, input_ids: list[int], target_ids: list[int], loss_mask: list[float]) -> tuple[float, dict]:
    _exp = math.exp
    _log = math.log
    _sum = sum
    ...
    for i in _rn:
        x_i = x[i]
        logits_i = [_sum(x_i[d] * lm_head[j][d] for d in _rD) for j in _rV]
    ...
    loss = -_sum(_log(probs[i][target_ids[i]]) * loss_mask[i] for i in _rn) * inv_sum_mask
...
def step_fn(params: dict, opt_state: dict, input_ids: list[int], target_ids: list[int], loss_mask: list[float], step: int) -> tuple[float, dict, dict]:
    ...
    for k in params:
        ...
        for i in range(nrow):
            ...
            new_p_i, new_m_i, new_v_i = new_p[i], new_m_mat[i], new_v_mat[i]
            for j in range(ncol):
                g = g_i[j]
                m_ij = beta1 * m_i[j] + one_minus_b1 * g
                v_ij = beta2 * v_i[j] + one_minus_b2 * g * g
                new_p_i[j] = p_i[j] - lr_scaled * m_ij / (_pow(v_ij * inv_bias2, 0.5) + eps)
                new_m_i[j] = m_ij
                new_v_i[j] = v_ij
        new_params[k], new_m[k], new_v[k] = new_p, new_m_mat, new_v_mat
